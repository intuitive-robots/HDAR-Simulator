...
class OpenDoorSimulator(SFSimulator):
    ...
    def reset(self):
        picked_box_euler = np.array([np.random.choice([45,-180]),0,0])
        picked_box_quat = R.from_euler("xyz", picked_box_euler).as_quat()
        picked_box_pos = [np.random.uniform(0.45, 0.7),np.random.uniform(0.7, -0.5),0]
        ...
        self.pick_robot.gotoCartPositionAndQuat(
            desiredPos=[self.picked_box.pos[0]-0.2, self.picked_box.pos[1]-0.2, 0.1],
        ...
        self.controller.setSetPoint(
            ...
            # np.hstack(([0.2, 0.2, 0.13], [0, 1, 0, 0]))
        )
