...
class BimanualHoldingSimulator(SFSimulator):
    ...
    def reset(self):
        ...
        self.pick_robot1.gotoCartPositionAndQuat(
            desiredPos=[self.picked_Sphere.pos[0], self.picked_Sphere.pos[1], 0.4],
            desiredQuat=[0, 1, 0, 0],
            duration=2.0,
        )
        self.pick_robot1.gotoCartPositionAndQuat(
            desiredPos=[self.picked_Sphere.pos[0], self.picked_Sphere.pos[1], 0.2],
            desiredQuat=[0, 1, 0, 0],
            duration=2.0,
        )
        self.pick_robot2.gotoCartPositionAndQuat(
            desiredPos=[self.target_Sphere.pos[0], self.target_Sphere.pos[1], 0.4],
            desiredQuat=[0, 1, 0, 0],
            duration=2.0,
        )
        self.pick_robot2.gotoCartPositionAndQuat(
            desiredPos=[self.target_Sphere.pos[0], self.target_Sphere.pos[1], 0.2],
            desiredQuat=[0, 1, 0, 0],
            duration=2.0,
        )
...
if __name__ == '__main__':
    simulator = BimanualHoldingSimulator()
    simulator.run()
