...
class HoldBoard(SFSimulator):
    ...
    def __init__(self, host_address=None):
        ...
        super().__init__('HoldBoard', host_address)
    ...
    def create_objects(self) -> Dict[str, MujocoObject]:
        self.picked_board = CustomMujocoObject(
            object_name="picked_board",
            object_dir_path=os.path.dirname(os.path.abspath(__file__)),
            pos=[0.4, 0.0, 0.3],
            quat=[0, 0, 0, 1],
        )
        self.target_board = CustomMujocoObject(
            object_name="target_board",
            object_dir_path=os.path.dirname(os.path.abspath(__file__)),
            pos=[0.4, -0.0, 0.0],
            quat=[0, 0, 0, 1],
        )
        return {
            "picked_board": self.picked_board,
    ...
    def reset(self):
        ...
        while True:
            ...
            if 0.2 < np.linalg.norm(
                ...
                break
        pushed_box_euler = np.array([np.random.uniform(-1.7, -1.4), 0, 0])
        ...
        self.mj_scene.set_obj_pos_and_quat(
            ...
            obj_name="picked_board",
        )
        target_box_pos= pushed_box_pos+np.array([0,0,0.15])
        target_box_euler = np.array([np.random.uniform(1.2, 1.8), 0, 0])
        target_box_quat = R.from_euler("xyz", target_box_euler).as_quat()
        self.mj_scene.set_obj_pos_and_quat(
            new_pos=target_box_pos,
            new_quat=target_box_quat,
            obj_name="target_board",
        )
        ...
        #     duration=0.5,
        # )
        current_pos1 = self.pick_robot1.current_c_pos
        current_pos2 = self.pick_robot2.current_c_pos
        home_pos = [0.53,-0.09,0.32]
        home_quat = [-0.03,0.75,0.67,0.01]
        # self.pick_robot1.gotoCartPositionAndQuat(
        #     desiredPos=[current_pos1[0], current_pos1[1], home_pos[2]],
        #     desiredQuat=home_quat,
        #     duration=0.2,
        # )
        # self.pick_robot2.gotoCartPositionAndQuat(
        #     desiredPos=[current_pos2[0], current_pos2[1], home_pos[2]],
        #     desiredQuat=home_quat,
        #     duration=0.2,
        # )
        self.pick_robot1.gotoCartPositionAndQuat(
...
class HoldBoardVibration(HoldBoard):
    def __init__(self, host_address=None):
    ...
    def after_step(self):
        ...
        target_pairs2={
            ...
            ('picked_handle2', 'finger2_rb0_tip_collision'),
        ...
        self.mj_scene,
        target_pairs1={
            ('picked_handle1', 'finger1_rb1_tip_collision'),
            ('picked_handle2', 'finger1_rb1_tip_collision'),
            
                       },
        target_pairs2={
            ('picked_handle1', 'finger2_rb1_tip_collision'),
            ('picked_handle2', 'finger2_rb1_tip_collision'),
