...
class PushCube(SFSimulator):
    ...
    def reset(self):
        ...
        while True:
            ...
            if np.linalg.norm(
                np.array(pushed_box_pos) - [0.53,-0.09,0.13]
            ) > 0.1 and np.linalg.norm(
                np.array(pushed_box_pos) - [0.5,0.25,0.13]
            ) > 0.2:
                break
        pushed_box_pos = [pushed_box_pos[0],pushed_box_pos[1],0.13]
    ...
    #simple task:
        ...
        # self.pick_robot.gotoCartPositionAndQuat(
        #     desiredPos=[home_pos[0], home_pos[1], home_pos[2]],
        #     desiredQuat=[0,1,0,0],
        #     duration=0.2,
        # )
        # self.pick_robot.gotoCartPositionAndQuat(
        #     desiredPos=[home_pos[0], home_pos[1], 0.28],
        #     desiredQuat=[0,1,0,0],
        #     duration=0.2,
        # )
        # inital_pos = home_pos
        # set_pos=[home_pos[0], home_pos[1], 0.28]
        # # set_pos= home_pos
        # set_quat = home_quat
        # self.pick_robot.activeController = self.controller_dict["panda_robot"]
        # self.pick_robot.activeController.setSetPoint(
        #     np.hstack((set_pos, [0,1,0,0]))
        # )
...
class PushCubeVibration(PushCube):
    ...
    def after_step(self):
        ...
        target_pairs1={
            ...
            ('pushed_box_a', 'panda_rb0_leftfinger:geom2'),
            ('pushed_box_b', 'panda_rb0_leftfinger:geom2'),
        ...
        target_pairs2={
            ...
            ('pushed_box_a', 'panda_rb0_rightfinger:geom2'),
            ('pushed_box_b', 'panda_rb0_rightfinger:geom2'),
            ('pushed_box_a', 'rod:geom_rb0'),
            ('pushed_box_b', 'rod:geom_rb0'),
            }
        )
